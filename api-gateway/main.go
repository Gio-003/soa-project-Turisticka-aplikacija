...
func main() {
	...
	muxRouter.Handle("/api/blogs/{blogId}/comments", middleware.JWTMiddlewareFunc(router.ProxyBlogWithReadCheck)).Methods("GET", "OPTIONS")
	muxRouter.Handle("/api/blogs/{blogId}/comments", middleware.JWTMiddlewareFunc(router.AddCommentWithFollowerCheck)).Methods("POST", "OPTIONS")
	...
	muxRouter.Handle("/api/blogs/welcome", middleware.JWTMiddlewareFunc(router.ProxyBlog)).Methods("POST", "OPTIONS")
	...
	muxRouter.Handle("/api/tours/{id}/archive", middleware.JWTMiddlewareFunc(handlers.ArchiveTourRPC)).Methods("POST", "OPTIONS")
	...
	muxRouter.Handle("/api/tours/draft/{userId}", middleware.JWTMiddlewareFunc(router.ProxyTour)).Methods("POST", "OPTIONS")
	muxRouter.Handle("/api/tours/{tourId}", middleware.JWTMiddlewareFunc(router.ProxyTour)).Methods("DELETE", "OPTIONS")

	// Tour Execution rute - SA JWT-om
	muxRouter.Handle("/api/tour-executions/start", middleware.JWTMiddlewareFunc(router.ProxyTour)).Methods("POST", "OPTIONS")
	muxRouter.Handle("/api/tour-executions/check-position", middleware.JWTMiddlewareFunc(router.ProxyTour)).Methods("POST", "OPTIONS")
	muxRouter.Handle("/api/tour-executions/{id}/complete", middleware.JWTMiddlewareFunc(router.ProxyTour)).Methods("POST", "OPTIONS")
	muxRouter.Handle("/api/tour-executions/{id}/abandon", middleware.JWTMiddlewareFunc(router.ProxyTour)).Methods("POST", "OPTIONS")
	muxRouter.Handle("/api/tour-executions/{id}", middleware.JWTMiddlewareFunc(router.ProxyTour)).Methods("GET", "OPTIONS")
	...
	// Purchase rute - SA JWT-om
	muxRouter.Handle("/api/cart", middleware.JWTMiddlewareFunc(router.ProxyPurchase)).Methods("GET", "OPTIONS")
	muxRouter.Handle("/api/cart/items/{tourId}", middleware.JWTMiddlewareFunc(router.ProxyPurchase)).Methods("POST", "DELETE", "OPTIONS")
	muxRouter.Handle("/api/cart/checkout", middleware.JWTMiddlewareFunc(router.ProxyPurchase)).Methods("POST", "OPTIONS")
	muxRouter.Handle("/api/purchases/{tourId}/token", middleware.JWTMiddlewareFunc(router.ProxyPurchase)).Methods("GET", "OPTIONS")
